        ...
        for sketch_dim in sketch_dim_vec:
            for lamda in lamda_vec:
                ...
                for iterate in range(repeat_time):
                    ...
                    # Gaussian
                    time_start=time()
                    model=SKSVM(sketch_method="GaussianSketch", sketch_dimension=sketch_dim,if_jit=False,lamda=lamda)
                    ...
                    # SRHT
                    time_start=time()
                    model=SKSVM(sketch_method="SRHT", sketch_dimension=sketch_dim,if_jit=False,lamda=lamda)
                    model.fit(X_train,Y_train)
                    pre_probability=model.predict(X_test)
                    time_end=time()
                    ...
                    # Count
                    time_start=time()
                    model=SKSVM(sketch_method="CountSketch", sketch_dimension=sketch_dim,if_jit=False,lamda=lamda)
                    model.fit(X_train,Y_train)
                    pre_probability=model.predict(X_test)
                    time_end=time()
                    ...
                    # Subsampling
                    time_start=time()
                    model=SKSVM(sketch_method="SubsamplingSketch", sketch_dimension=sketch_dim,if_jit=False,lamda=lamda)
                    model.fit(X_train,Y_train)
                    pre_probability=model.predict(X_test)
                    time_end=time()
